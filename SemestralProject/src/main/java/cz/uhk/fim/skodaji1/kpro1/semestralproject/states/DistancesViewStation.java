...
public class DistancesViewStation extends State
{
    ...
    {
       Station s = cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Stations.GetInstance().GetStation(data.get("station"));
       if (s != null)
       {
           data.put("station_from", s.GetName() + " (" + s.GetAbbrevation() + ")");
           data.put("stations_distances_tr", cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Distances.GetInstance().GenerateDistancesRows(s));
           this.origin = s;
