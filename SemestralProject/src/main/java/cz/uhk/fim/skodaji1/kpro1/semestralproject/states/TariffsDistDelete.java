...
public class TariffsDistDelete extends State {
    ...
    {
        this.tariff =(DistanceTariff) cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Tariffs.GetInstance().GetTariff(data.get("tariff_abbr"));
        if (tariff != null)
        {
            int min = -1, max = 1;
            for (Station from: cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Stations.GetInstance().GetAllStations())
            {
                for (Station to: cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Stations.GetInstance().GetAllStations())
                {
                    int dist = cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Distances.GetInstance().GetDistance(from, to);
                    if (min == -1)
                    {
                        min = dist;
                    }
    ...
    public void HandleInput(String input)
    {
        switch(input.toLowerCase())
        {
            ...
            case "yes":
                ...
                cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Tariffs.GetInstance().RemoveTariff(this.tariff);
