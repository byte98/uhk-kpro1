...
import cz.uhk.fim.skodaji1.kpro1.semestralproject.screens.ScreenFactory;
...
public class Welcome extends State
{
    ...
    public void HandleInput(String input)
    {
        switch (input.toLowerCase())
        {
            ...
            case "ticket":
                if (cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Tariffs.GetInstance().GetAllTariffs().length < 1)
                {
                    this.controller.ShowError("Nenalezen zadny tarif! Rezim prodeje nelze spustit!");
                }
                else if (cz.uhk.fim.skodaji1.kpro1.semestralproject.data.Stations.GetInstance().GetAllStations().length < 1)
                {
